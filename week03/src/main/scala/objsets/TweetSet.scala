...
abstract class TweetSet {
  ...
  def descendingByRetweet: TweetList = {
    def loop(tweetSet: TweetSet, tweetList: TweetList): TweetList = {
      if (tweetSet.isEmpty) tweetList
      else {
        val mostPopular = tweetSet.mostRetweeted
        new Cons(mostPopular, loop(tweetSet.remove(mostPopular), tweetList))
      }
    }
