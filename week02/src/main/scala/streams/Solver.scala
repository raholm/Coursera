  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val newStates = for {
        ...
        nextState <- newNeighborsOnly(neighborsWithHistory(block, history), explored)
      } yield nextState

      val newExploredBlocks = newStates map { case (block, _) => block }

      initial ++ from(newStates, explored ++ newExploredBlocks)
    }
  }
